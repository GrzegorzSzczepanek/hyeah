...
class HistoryManager:
    ...
    def delete_user(self, username):
        ...
        if user_id:
            with self.conn:
                ...
                self.conn.execute(
                    "DELETE FROM chat_history WHERE session_id IN (SELECT session_id FROM sessions WHERE user_id = ?)",
                    (user_id,),
                )
    ...
    def clear_session(self, username, session_name):
        ...
        if session_id:
            ...
            print(
                f"All messages cleared for session '{session_name}' of user '{username}'."
            )
        else:
            print(f"Session '{session_name}' or user '{username}' does not exist.")
...
def exampleUsage():
    ...
    manager.add_message(
        "john_doe", "Session 1", "Do you need assistance with your taxes?"
    )
